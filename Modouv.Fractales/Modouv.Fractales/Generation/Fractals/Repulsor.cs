...
namespace Modouv.Fractales.Generation.Fractals
{
    ...
    public static class Repulsor
    {
        ...
        /// </summary>
        static Effect m_repulsorEffect;
        /// <summary>
        /// Charge les composants nécessaires à l'utilisation ultérieure de la classe.
        /// </summary>
        public static void Initialize()
        {
            m_repulsorEffect = Game1.Instance.Content.Load<Effect>("Shaders\\fractals\\repulsor");
        }
        ...
        public static void GenerateTexture2DGPU(Complex c, Complex origin, float scale, RenderTarget2D renderTexture)
        {
            ...
            m_repulsorEffect.Parameters["width"].SetValue(width);
            m_repulsorEffect.Parameters["height"].SetValue(height);
            m_repulsorEffect.Parameters["scale"].SetValue(scale);
            m_repulsorEffect.Parameters["colorParam"].SetValue(new float[] { c.Real, c.Imaginary }); // conseillée 0.5 / 0.5
            m_repulsorEffect.Parameters["origin"].SetValue(new float[] { origin.Real, origin.Imaginary });
            m_repulsorEffect.Parameters["MatrixTransform"].SetValue(Game1.Instance.PlaneTransform2D);
            ...
            m_repulsorEffect.CurrentTechnique.Passes[0].Apply();
