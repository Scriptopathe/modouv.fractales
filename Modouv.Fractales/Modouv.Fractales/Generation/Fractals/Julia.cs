...
namespace Modouv.Fractales.Generation.Fractals
{
    ...
    public static class Julia
    {
        ...
        {
            m_juliaEffect = Game1.Instance.Content.Load<Effect>("Shaders\\fractals\\julia");
        }
        ...
        public static void GenerateTexture2DGPU(Complex c, Complex origin, float scale, RenderTarget2D renderTexture, string technique="Colored")
        {
            int width = renderTexture.Width;
            int height = renderTexture.Height;

            // Passe les paramètres de la fractale au pixel shader.
            m_juliaEffect.CurrentTechnique = m_juliaEffect.Techniques[technique];
        ...
        const int CORES = 4;
        public const int JULIA_MAX_DEPTH = 150;
        /// <summary>
        /// Génère une heightmap à l'aide d'une fractale de Julia.
        ...
        /// <returns></returns>
        public static float[,] GenerateJuliaHeightmap(Complex c, int width, int height, Complex origin, float scale)
        {
            ...
            ParameterizedThreadStart start = new ParameterizedThreadStart((object rectangle) =>
            {
                ...
                for (int x = rect.X; x < rect.Right; x++)
                {
                    ...
                    for (int y = rect.Y; y < rect.Bottom; y++)
                    {
                        ...
                        lock (mutex)
                        {
                            data[x, y] = (float)depth/(float)JULIA_MAX_DEPTH;
                        }
                    }
                }
            });
